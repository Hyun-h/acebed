<?php 
    ...
    $borde_num = $_GET["num"]; //클릭한 n번째 숫자. 몇번째 리스트로 넘기는지 ex)2번째
    
    $sql = "SELECT * FROM acebed_board WHERE num = " . $borde_num;
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_array($result);
    ...
    $item_desc = $row["descriptions"]; //array로 넣은걸 문자열로 변환시켜줘야 함
    $item_descs = nl2br($row["descriptions"]); 
    $item_date = $row["dates"];

    //or $item_desc = $row["descriptions"]; //array로 넣은걸 문자열로 변환시켜줘야 함
    //or and $item_descs = json_decode($item_desc);
?>
